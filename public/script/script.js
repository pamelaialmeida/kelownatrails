...
STUDENT NAME: PAMELA INACIO ALMEIDA
STUDENT ID: CT1004057
SQAC114
==========================================
...
function RemoveGroupMember() {
	...
	if (membersLst.options.length > 0) {
		let memberSelected = membersLst.selectedIndex;

		if (memberSelected === -1) {
			throw "Please, select a member to delete from the list.";
		} else {
			let memberName = membersLst.options[memberSelected].text;

			membersLst.remove(memberSelected);			
			alert(`The member ${memberName} was removed from the list.`);
		}

	} else {
		throw "There are no members in the list.";
	}
